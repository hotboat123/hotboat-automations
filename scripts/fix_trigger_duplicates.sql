-- Script para evitar que el trigger procese la misma reserva múltiples veces
-- Añade validación para que solo cree consumos si no existen ya para esa reserva
...
BEGIN
    -- VALIDACIÓN: Verificar si ya existen consumos para esta reserva
    SELECT COUNT(*) INTO v_existing_count
    FROM reservation_consumption
    WHERE reservation_id = NEW.id::text;
    
    -- Si ya existen consumos para esta reserva, no crear duplicados
    IF v_existing_count > 0 THEN
        RAISE NOTICE 'Reserva % ya tiene consumos registrados, omitiendo', NEW.id;
        RETURN NEW;
    END IF;
